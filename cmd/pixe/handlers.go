...
func handleIndex() {
	...
	// Build index - use basename for memoryID to avoid path issues
	...
	if jsonOutput {
		result := map[string]interface{}{
			...
			"indexPath":   filepath.Join(indexDir, memoryID+".index"),
		}
		...
		fmt.Printf("  Index saved to: %s/%s.index\n", indexDir, memoryID)
	}
}
...
func handleSearch() {
	...
	if err != nil {
		if jsonOutput {
			fmt.Fprintf(os.Stdout, `{"error":%q,"results":[]}\n`, err.Error())
		} else {
			fmt.Fprintf(os.Stderr, "Error creating indexer: %v\n", err)
		}
		os.Exit(1)
	...
	// Load index - use basename for memoryID
	memoryID := filepath.Base(inputPath)
...
func handleChat() {
	...
	// Check if index exists - use basename for memoryID
	memoryID := filepath.Base(inputPath)
	idx, err := indexer.LoadIndex(memoryID)
	if err != nil {
