...
const (
	// Local inference (no cloud, no cost)
	ModelLocal = "local" // Local model (e.g. DeepSeek R1 32B) via llama.cpp / Ollama

	// Top tier - Latest models (via OpenRouter)
...
// GetDefaultModel returns the default model — local inference (free, private, fast).
func GetDefaultModel() string {
	return ModelLocal // Local inference, $0/token, full privacy
}
