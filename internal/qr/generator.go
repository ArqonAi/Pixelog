...
func (g *Generator) GenerateFrames(chunks []Chunk) ([]string, error) {
	...
	for i, chunk := range chunks {
		...
		hints[gozxing.EncodeHintType_ERROR_CORRECTION] = "L"
		bitMatrix, err := writer.Encode(string(chunkData), gozxing.BarcodeFormat_QR_CODE, 512, 512, hints)
		if err != nil {
			return nil, fmt.Errorf("failed to encode QR code for chunk %d (payload %d bytes): %w", i, len(chunkData), err)
		}
...
func (g *Generator) GenerateFrame(chunk Chunk, frameNumber int) (string, error) {
	...
	bitMatrix, err := writer.Encode(string(chunkData), gozxing.BarcodeFormat_QR_CODE, 512, 512, hints)
	if err != nil {
		return "", fmt.Errorf("failed to encode QR code (payload %d bytes): %w", len(chunkData), err)
	}
