...
func (g *Generator) GenerateFrames(chunks []Chunk) ([]string, error) {
	...
	for i, chunk := range chunks {
		...
		// Generate QR code with gozxing (same library for encode/decode = guaranteed compatibility)
		framePath := filepath.Join(g.outputDir, fmt.Sprintf("frame_%05d.png", i))
		
		writer := qrcode.NewQRCodeWriter()
		hints := make(map[gozxing.EncodeHintType]interface{})
		hints[gozxing.EncodeHintType_ERROR_CORRECTION] = "M" // Medium error correction
...
func DecodeFrame(imagePath string) (*Chunk, error) {
	// Use gozxing (pure Go, same library as encoder = guaranteed compatibility)
	...
	// Create binary bitmap for gozxing
	bmp, err := gozxing.NewBinaryBitmapFromImage(img)
	if err != nil {
		return nil, fmt.Errorf("failed to create bitmap: %w", err)
	}

	// Decode QR code with PURE_BARCODE hint for better reliability
